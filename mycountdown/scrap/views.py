...
def retday():
    ...
    target_time_zone = pytz.timezone('Etc/GMT+2')
    racetime_utc= race_datetime_adjusted.astimezone(target_time_zone)
    print(racetime_utc)
    driver.close()
    return racetime_utc, relevant_dictionary
    # while(True):
    #     pass
...
def show_countdown(request):
    race_time, rel_dic= retday()
    mycontext = {
        'timee':race_time.strftime("%B %d, %Y %H:%M:%S"),
        'Country':rel_dic["Country"],
        'RaceDate':rel_dic["RaceDate"],
        'RaceTime':rel_dic["RaceTime"],
        'Month': rel_dic["Month"].capitalize
    }

    print(race_time.strftime("%B /%d, /%Y %H:%M:%S"))
